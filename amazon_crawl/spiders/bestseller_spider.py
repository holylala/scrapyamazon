...
class BestsellerSinglePageSpider(scrapy.Spider):
    ...
    # scrapy 调式模式： scrapy shell "https://www.amazon.cn/gp/bestsellers/books/ref=zg_bs_books_pg_1?ie=UTF8&pg=1"
    def parse(self, response):
        div_zg_itemRow_lst = response.xpath('//div[@class="zg_itemRow"]')
        
        for div_zg_itemRow in div_zg_itemRow_lst:
            item = BestsellerItem()
            item['rank_number'] = div_zg_itemRow.xpath('.//span[@class="zg_rankNumber"]/text()').extract_first() #当前节点下 返回的是selector 只取第一个
